...
type Raft struct {
	...
	jobsChan            chan RaftLog
	rpcChan             chan int
	heartBeatChan       []chan struct{}
	heartBeatTimerReset chan bool
	readyChan           []chan AppendEntriesArgs
	// leaderBeginChan     chan bool
	termChangeChan chan int32
	stopChan       chan bool
	commitIdxChan  chan int32
	logidx         *At32
	isLeader       *AtBool
...
func (rf *Raft) RequestVote(args RequestVoteArgs, reply *RequestVoteReply) {
	...
	if args.Term > currentTerm || (currentTerm == args.Term && rf.VotedFor == -1) {
		...
		if mylastterm < args.Log.Term || (mylastterm == args.Log.Term && mylastidx <= args.Log.Index) {
			reply.Voted = true
			rf.termChangeChan <- args.Term
			rf.VotedFor = args.ID
			// rf.leaderBeginChan <- false
...
func (rf *Raft) sendHeartBeat(server int, args AppendEntriesArgs, reply *AppendEntriesReply) bool {
	okChan := make(chan bool, 1)
	go func() {
		okChan <- rf.peers[server].Call("Raft.HeartBeat", args, reply)
	}()
	var ok bool
	select {
	case ok = <-okChan:
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	rf.logidx = &At32{}
	rf.isLeader = &AtBool{}
	rf.stopChan = make(chan bool, 1)
	rf.logs = make([]RaftLog, 1)
	rf.rpcChan = make(chan int, MAX_LOGS)
	rf.heartBeatChan = make([]chan struct{}, len(rf.peers))
	for i := range rf.heartBeatChan {
		rf.heartBeatChan[i] = make(chan struct{}, 1)
	}
	rf.readyChan = make([]chan AppendEntriesArgs, len(rf.peers))
	for i := range rf.readyChan {
		rf.readyChan[i] = make(chan AppendEntriesArgs, MAX_LOGS)
	}
	rf.jobsChan = make(chan RaftLog, MAX_LOGS)
	...
	// rf.leaderBeginChan = make(chan bool, 1)
	rf.termChangeChan = make(chan int32, len(rf.peers))
	rf.commitIdxChan = make(chan int32, 512)
		...
		for {
			...
			case <-rf.heartbeatTimer.C:
				...
				for i := 0; i < lengthPeers; i++ {
					...
					//
					if len(rf.readyChan[i]) != 0 || i == int(rf.me) {
						continue
					}
	...
	// All incoming entries will go to jobsChan
	...
	go func(rf *Raft) {
		...
		for {
			...
			case <-time.After(JobChannelPollRate()):
				select {
				case job := <-rf.jobsChan:
					numJobs := len(rf.jobsChan) + 1
	...
	go func(rf *Raft) {
		for i := 0; i < len(rf.peers); i++ {
			...
			go func(server int, readyChan <-chan AppendEntriesArgs) {
				for {
					select {
					case <-rf.stopChan:
						rf.stopChan <- true
						return
					case appendArg := <-readyChan:
	...
	go func(rf *Raft) {
		...
		for {
			...
			// select {
			// case leader := <-rf.leaderBeginChan:
			...
			case <-rf.electionTimer.C:
				...
				votingChan := make(chan bool, len(rf.peers)-1)
				...
				go func(currentTerm int32, votingChan <-chan bool) {
					// Check votes
					votes := 0
					...
					for {
						...
						case isVoted := <-votingChan:
							...
							if votes == len(rf.peers)/2 {
								...
								// rf.leaderBeginChan <- true
								// Becoming leader
								for i := 0; i < len(rf.peers); i++ {
									if int(rf.me) == i {
										continue
									}
									rf.readyChan[i] <- AppendEntriesArgs{
										Term:            currentTerm,
										LeaderID:        rf.me,
										PrevLog:         lastLog,
										Entries:         nil,
				...
				}(currentTerm, votingChan)

				for i := 0; i < lengthPeers; i++ {
					if i != int(rf.me) {
						// request vote rpc for each peer that is not me
						go func(server int, args RequestVoteArgs, votingChan chan bool) {
